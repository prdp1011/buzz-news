...
import { FetchFreshStoriesButton } from "@/components/FetchFreshStoriesButton";
...
export default async function PostsPage() {
  ...
  const [posts, sources] = await Promise.all([
    ...
    prisma.source.findMany({
      orderBy: { name: "asc" },
      select: { id: true, name: true, type: true, feedUrl: true, isActive: true },
    }),
  ]);
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <FetchFreshStoriesButton sources={sources} />
